...
;(function (b) {
  let m; let t; let u; let f; let D; let j; let E; let n; let z; let A; let q = 0; let e = {}; let o = []; let p = 0; let d = {}; let l = []; let G = null; let v = new Image(); const J = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i; const W = /[^\.]\.(swf)\s*$/i; let K; let L = 1; let y = 0; let s = ''; let r; let i; let h = false; const B = b.extend(b('<div/>')[0], { prop: 0 }); const M = b.browser.msie && b.browser.version < 7 && !window.XMLHttpRequest; const N = function () { t.hide(); v.onerror = v.onload = null; G && G.abort(); m.empty() }; const O = function () {
    if (e.onError(o, q, e) === false) { t.hide(); h = false } else {
    ...
    let a = o[q]; let c; let g; let k; let C; let P; let w; N(); e = b.extend({}, b.fn.fancybox.defaults, typeof b(a).data('fancybox') === 'undefined' ? e : b(a).data('fancybox')); w = e.onStart(o, q, e); if (w === false)h = false; else {
...
'function') { w = e.ajax.win(c, x, H, R); if (w === false) { t.hide(); return } else if (typeof w === 'string' || typeof w === 'object')x = w }m.html(x); F()
    ...
    let a = e.width; let c = e.height; a = a.toString().indexOf('%') > -1 ? parseInt((b(window).width() - e.margin * 2) * parseFloat(a) / 100, 10) + 'px' : a == 'auto' ? 'auto' : a + 'px'; c = c.toString().indexOf('%') > -1 ? parseInt((b(window).height() - e.margin * 2) * parseFloat(c) / 100, 10) + 'px' : c == 'auto' ? 'auto' : c + 'px'; m.wrapInner('<div style="width:' + a + ';height:' + c +
';overflow: ' + (e.scrolling == 'auto' ? 'auto' : e.scrolling == 'yes' ? 'scroll' : 'hidden') + ';position:relative;"></div>'); e.width = m.width(); e.height = m.height(); Q()
  }; var Q = function () {
    let a, c; t.hide(); if (f.is(':visible') && d.onCleanup(l, p, d) === false) { b.event.trigger('fancybox-cancel'); h = false } else {
      h = true; b(j.add(u)).unbind(); b(window).unbind('resize.fb scroll.fb'); b(document).unbind('keydown.fb'); f.is(':visible') && d.titlePosition !== 'outside' && f.css('height', f.height()); l = o; p = q; d = e; if (d.overlayShow) {
        ...
        if (b.isFunction(d.titleFormat))a = d.titleFormat(s, l, p, d); else {
          a = s && s.length
            ? d.titlePosition == 'float' ? '<table id="fancybox-title-float-wrap" cellpadding="0" cellspacing="0"><tr><td id="fancybox-title-float-left"></td><td id="fancybox-title-float-main">' + s + '</td><td id="fancybox-title-float-right"></td></tr></table>' : '<div id="fancybox-title-' + d.titlePosition + '">' + s + '</div>'
            : false
        }s = a; if (!(!s || s === '')) {
...
37 || a.keyCode == 39) && d.enableKeyboardNav && a.target.tagName !== 'INPUT' && a.target.tagName !== 'TEXTAREA' && a.target.tagName !== 'SELECT') { a.preventDefault(); b.fancybox[a.keyCode == 37 ? 'prev' : 'next']() }
    ...
    f.show(); h = false; b.fancybox.center(); d.onComplete(l, p, d); let a, c; if (l.length - 1 > p) { a = l[p + 1].href; if (typeof a !== 'undefined' && a.match(J)) { c = new Image(); c.src = a } } if (p > 0) { a = l[p - 1].href; if (typeof a !== 'undefined' && a.match(J)) { c = new Image(); c.src = a } }
  }; var T = function (a) {
    const c = { width: parseInt(r.width + (i.width - r.width) * a, 10), height: parseInt(r.height + (i.height - r.height) * a, 10), top: parseInt(r.top + (i.top - r.top) * a, 10), left: parseInt(r.left + (i.left - r.left) * a, 10) }; if (typeof i.opacity !== 'undefined')c.opacity = a < 0.5 ? 0.5 : a; f.css(c)
    j.css({ width: c.width - d.padding * 2, height: c.height - y * a - d.padding * 2 })
  }; const U = function () { return [b(window).width() - d.margin * 2, b(window).height() - d.margin * 2, b(document).scrollLeft() + d.margin, b(document).scrollTop() + d.margin] }; var X = function () {
    const a = U(); const c = {}; let g = d.autoScale; const k = d.padding * 2; c.width = d.width.toString().indexOf('%') > -1 ? parseInt(a[0] * parseFloat(d.width) / 100, 10) : d.width + k; c.height = d.height.toString().indexOf('%') > -1 ? parseInt(a[1] * parseFloat(d.height) / 100, 10) : d.height + k; if (g && (c.width > a[0] || c.height > a[1])) {
      if (e.type ==
'image' || e.type == 'swf') { g = d.width / d.height; if (c.width > a[0]) { c.width = a[0]; c.height = parseInt((c.width - k) / g + k, 10) } if (c.height > a[1]) { c.height = a[1]; c.width = parseInt((c.height - k) * g + k, 10) } } else { c.width = Math.min(c.width, a[0]); c.height = Math.min(c.height, a[1]) }
    ...
    let a = e.orig ? b(e.orig) : false; let c = {}; if (a && a.length) {
  ...
  }; const Z = function () { if (t.is(':visible')) { b('div', t).css('top', L * -40 + 'px'); L = (L + 1) % 12 } else clearInterval(K) }
...
1)
    ...
    function a () { u.fadeOut('fast'); n.empty().hide(); f.hide(); b.event.trigger('fancybox-cleanup'); j.empty(); d.onClosed(l, p, d); l = e = []; p = q = 0; d = e = {}; h = false } if (!(h || f.is(':hidden'))) {
      h =
true; if (d && d.onCleanup(l, p, d) === false)h = false; else {
        N(); b(E.add(z).add(A)).hide(); b(j.add(u)).unbind(); b(window).unbind('resize.fb scroll.fb'); b(document).unbind('keydown.fb'); j.find('iframe').attr('src', M && /^https/i.test(window.location.href || '') ? 'javascript:void(false)' : 'about:blank'); d.titlePosition !== 'inside' && n.empty(); f.stop(); if (d.transitionOut == 'elastic') {
          r = V(); const c = f.position(); i = { top: c.top, left: c.left, width: f.width(), height: f.height() }; if (d.opacity)i.opacity = 1; n.empty().hide(); B.prop = 1
